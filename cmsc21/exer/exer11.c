 ...
 *It also saves and loads contacts list in a file
 */
...
// function that saves the list to a file
void save(Node *head) {
  ...
  if(fp) {
    if(p == head) {
      ...
      system("rm contacts.txt"); // deletes the file
      return;
    }
    while(p != head) { // traverse and saves the contacts
      if(p->next != head) {
	fprintf(fp, "%s %lli", p->name, p->num); 
	fprintf(fp, "%c", '\n');
      } else {
	fprintf(fp, "%s %lli", p->name, p->num);
      }
...
void load(Node *head) {
  ...
  if(fp) {
    if(feof(fp)) { // if nothing is in the list
      return;
    }
    while(!feof(fp)) {
      newNode = createNode();
      fscanf(fp, "%s %lli", newNode->name, &newNode->num); // loads the list from the file
      // linking
      newNode->next = head; 
...
void insert(Node *head) {
  ...
  Node *p = head->next;
  while(p != head && strcmp(newNode->name, p->name) > 0) {
    p = p->next;
  }
  // linking
  newNode->next = p;
