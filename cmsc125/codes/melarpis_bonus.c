 ...
 * CMSC 125 ST-8L
 * BONUS
 * This program implements a Preemptive Shortest Job First (SJF) CPU Scheduler. 
...
/* function that sorts the ready queue per cycle */
void update()
{
  linked_list ptr = ready_queue;
  linked_list temp = ready_queue;
  if(ready_queue != NULL && ready_queue->next != NULL) /* there are at least 2 jobs in the ready queue */
  {
    /* sort the ready queue */
    while(ptr->next != NULL && ptr->next->pcb.job_length <= ready_queue->pcb.job_length)
    {
      ptr = ptr->next;
    }
    if(ptr->next == NULL)
...
RunSimulation()
{
	...
	while(1)
	{
		...
		Processing();
    update(); /* check arrangement of jobs in the ready queue if still sorted */
		cpu_cycles++;
